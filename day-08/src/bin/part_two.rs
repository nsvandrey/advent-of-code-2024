...
impl PointPair {
    ...
    fn antinodes(self: &Self, grid: &Vec<Vec<char>>) -> Vec<[isize; 2]> {
        ...
        if self.point_a[1] > self.point_b[1] {
            loop {
                let antinode_a = [point_a[0] + distance[0], point_a[1] + distance[1]];
                if on_grid(grid, antinode_a) == false {
                    break;
                }
            ...
            loop {
                let antinode_b = [point_b[0] - distance[0], point_b[1] - distance[1]];
                if on_grid(grid, antinode_b) == false {
                    break;
                }
        ...
        antinodes.push(self.point_a);
        antinodes.push(self.point_b);
